...
export default function AppSelect({
  ...
  return (
    <div
      className={`flex flex-col text-dark text-xs lg:text-sm mt-5 mb-2 z-[2000] ${className}`}
    >
      <label>
        {label}
        {isRequired && <span className="text-danger text-lg">*</span>}
      </label>
      <Listbox
        value={values[name]}
      ...
      >
        <div className="relative mt-1 h-10">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute z-20 w-full py-1 mt-1 overflow-auto text-xs lg:text-sm bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
